...
function run() {
	if (!graph || !config) {
		console.log('Please init graph first');
	}
...
function highlightAnalogy(analogy) {
	...
	graph.node.classed('analogy-source', function (d) {
		if (d.ref == analogy.source)
			return true;
		return false;
	});
	graph.node.classed('analogy-target', function (d) {
		if (d.ref == analogy.target)
			return true;
		return false;
	});
	graph.node.classed('inactive', function (d) {
	...
	graph.line.classed('inactive', function (d) {
		if ((d.source.ref == analogy.source || d.source.ref == analogy.target) && (src_evidence[String(d.target.ref)] || trg_evidence[String(d.target.ref)]))
			return false;
		return true;
	});
	graph.line.classed('analogy-src-link', function (d) {
		if (d.source.ref == analogy.source && src_evidence[String(d.target.ref)])
			return true;
		return false;
	});
	graph.line.classed('analogy-trg-link', function (d) {
		if (d.source.ref == analogy.target && trg_evidence[String(d.target.ref)])
			return true;
		return false;
	});
}
